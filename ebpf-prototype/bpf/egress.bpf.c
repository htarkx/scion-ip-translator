...
static inline __u32 write_scionhdr(void *buf, struct scionhdr *hdr,
                                   struct ipv6hdr *iph) {
  ...
  SC_SET_QOS(new_hdr,
             ((__u32)((iph->priority << 4) | (iph->flow_lbl[0] >> 4))));
  // 20b copy flow label from IP header
  SC_SET_FLOW(new_hdr,
              ((((__u32)iph->flow_lbl[0] & 0xF) << 16) |
               ((__u32)iph->flow_lbl[1] << 8) | (__u32)iph->flow_lbl[2]));
...
///
/// Returns number of written bytes
static inline __u32 write_host_addr(void *buf, struct in6_addr *daddr,
...
static inline int adjust_eth(struct __sk_buff *ctx, struct ethhdr *eth,
                             struct ipv6hdr *iph) {
...
int scion_egress(struct __sk_buff *ctx) {
  ...
  // Lookup destination SCION IA from CIDR map.
  struct lpm_key_v6 lpm_key = {.prefixlen = 128};
  __builtin_memcpy(&lpm_key.addr, &ip6_hdr->daddr, sizeof(lpm_key.addr));
  __u32 *dst_ptr = bpf_map_lookup_elem(&ipv6_to_scion, &lpm_key);
  if (!dst_ptr)
    return TC_ACT_OK;
  dst = *dst_ptr;
  ...
  // We add the size of the headers to be inserted in order to retrieve
  // information from the orignal header
  sci_end = sci_hdr;
  ...
  // The required fields are written later, which is why we have to adjust the
  // fields now.
  ip6_hdr->payload_len =
      bpf_htons((__u16)sizeof(struct udphdr) + (4 * (__u16)sci_hdr->len) +
